...
#ifndef _AMU_ABI_MULTI_BINARY_H_
#define _AMU_ABI_MULTI_BINARY_H_

#include "amuABI.h"
#include "amuABIElf.h"
#include "cal_stl_vector.h"

class AMUabiEncodingPackData
{
public:
    ...
    unsigned int notesOff;
    unsigned int notesSize;
    ...
    // Constant data
    unsigned int float32ConstsCount;
    CALDataSegmentDesc* float32Consts;
    unsigned int int32ConstsCount;
    CALDataSegmentDesc* int32Consts;
    unsigned int bool32ConstsCount;
    CALDataSegmentDesc* bool32Consts;
}; 

class AMUabiEncodingUnpackData
{
public:
    ...
    // Temporary storage for unpacked objects 
    AMUabiLiteralConst *litConstsData;
    AMUabiUserSymbol *symbolsData;
}; 

class AMUabiEncodingSignature
{
public:
    AMUabiEncodingSignature();
    ~AMUabiEncodingSignature();

    bool operator==(const AMUabiEncodingSignature &rhs)
    {
        return ((machine == rhs.machine) && (type == rhs.type));
    }
...
class AMUabiEncodingInfo
{
public:
    AMUabiEncodingInfo();
    AMUabiEncodingInfo(const AMUabiEncodingInfo& src);

    ~AMUabiEncodingInfo();

    //////////////////////////////////////////////////////////////
    //
    // Identifying signature for the encoding
    //
    AMUabiEncodingSignature signature;

    //////////////////////////////////////////////////////////////
    //
    // User accessible values for the encoding
    //
    ...
    unsigned int globalBuffersCount;
    unsigned int *globalBuffers;
    unsigned int scratchBuffersCount;
    unsigned int *scratchBuffers;
    unsigned int persistentBuffersCount;
    unsigned int *persistentBuffers;
    unsigned int litConstsCount;
    AMUabiLiteralConst* litConsts;
    unsigned int constBuffersCount; 
    CALConstantBufferMask* constBuffers;
    unsigned int inputSamplerMapCount;
    CALSamplerMapEntry* inputSamplerMaps;
    unsigned int progInfosCount;
    CALProgramInfoEntry* progInfos;
    unsigned int textSize;
    void* textData;
    unsigned int symbolsCount;
    AMUabiUserSymbol* symbols;
    unsigned int subBufferCount;
    CALSubConstantBufferEntry* subBuffers;

    //////////////////////////////////////////////////////////////
    //
    // Temporary values needed for packing
    //
    AMUabiEncodingPackData packData;

    //////////////////////////////////////////////////////////////
    //
    // Temporary values needed for unpacking
    //
    AMUabiEncodingUnpackData unpackData;
};

class AMUabiMultiBinaryInfo
{
public:
    AMUabiMultiBinaryInfo();
    ~AMUabiMultiBinaryInfo();

    void DeleteEncodings();

    calstd::vector<AMUabiEncodingInfo*> encodings;
};

#endif // _AMU_ABI_MULTI_BINARY_H_
