...
AST::DelayedLookup::EffectiveDataType()
{
  ASSERT_WARNING(GetResultSymbol()!=NULL)(*mName).Text("DataType still undefined");
  static DataTypePtr invalid;
  if(GetResultSymbol()==0) return invalid;
  return GetResultSymbol()->Type();
}

std::string 
AST::DelayedLookup::DebugStatus() const
{
  if(GetResultSymbol()==0)
    return "???";
  else
    return GetResultSymbol()->ToString();
}

// int 
// AST::DelayedLookup::PostAllocateRegisters( TreeElementPtr& self, ILTranscriber* il )
// {
//   SetResultSymbol(mSymbolValue);
//   return 0;
// }

AST::DelayedLookup& AST::DelayedLookup::AsDelayedLookup()
{
  return *this;
}

int 
AST::DelayedLookup::ResolveDataTypes( TreeElementPtr& /*self*/, DataTypeTable*& scope )
{
  if(mDataTypeValue.IsValid()) return 0;

  mDataTypeValue = scope->Lookup(mName);

  return 0;
}

int 
AST::DelayedLookup::ResolveSymbols( TreeElementPtr& /*self*/, SymbolTable*& scope )
{
  if(GetResultSymbol().IsValid()) return 0;

  SetResultSymbol(scope->Lookup(mName));

  ASSERT_WARNING(GetResultSymbol().IsValid() + mDataTypeValue.IsValid() >= 1)(*mName).Text("unknown symbol");
  ASSERT_WARNING(GetResultSymbol().IsValid() + mDataTypeValue.IsValid() <= 1)(*mName).Text("could be either a function or data type");

  //return 1 if we are undefined
  return GetResultSymbol().IsValid() + mDataTypeValue.IsValid() != 1;
}
