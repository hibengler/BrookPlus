...
#ifndef AMD_HLFRONTENDAPI_HPP
#define AMD_HLFRONTENDAPI_HPP
...
extern "C"
{
  ...
  /// Run compiler with the given commend line args. (Called in hlfrontend.exe)
  bool HLFrontendRun(int argc, const char** argv);


  namespace HLFrontendFlags
  {
    enum ShaderType   { TYPE_FROMPARAMS, TYPE_PS, TYPE_VS, TYPE_GS, TYPE_CS };
    enum OutputFormat {
        OUTPUT_FROMPARAMS = 1,
        OUTPUT_TXT = 2,
        OUTPUT_BIN = 3,
    ...
    //note: *_FROMPARAMS indicates behavior should be derived from the state
    //      of the "ps", "vs", and "gs" / "txt", "bin", and "hex" parameters
  };


  /// Compile a provided buffer using the given flags returns false on error
  bool HLFrontendCompileBuffer( std::istream& input
                              , std::ostream& output
                              , HLFrontendFlags::ShaderType type
                              , HLFrontendFlags::OutputFormat fmt = HLFrontendFlags::OUTPUT_BIN
                              , ConstShaderDescriptionPtr* shaderDesc = NULL);

  /// Reset all parameters to default state
  void HLFrontendResetParameters();

  /// Toggle HLFrontend parameters
  void HLFrontendSetParameter(const char* name, bool enabled, const char* val = "");

  /// Set error msg handler; defaults to std::cerr 
