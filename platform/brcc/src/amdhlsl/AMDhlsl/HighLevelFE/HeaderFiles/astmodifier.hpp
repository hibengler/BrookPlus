...
#ifndef AMD_ASTMODIFIER_HPP
#define AMD_ASTMODIFIER_HPP
...
class Modifier : public ConstStringPtr
{
public:
  enum ModifierType
  {
  ...
  Modifier(const ConstStringPtr& name, ModifierType type);

  Modifier() : mType(MOD_INVALID), mNumber(-1) {}

  ModifierType Type() const { return mType; }

  ILSpec::ILImportUsage LookupSemanticUsage() const;
  ILSpec::ILRegType LookupInputSemanticRegister() const; // for input semantics  

  int  Number() const { return mNumber; }
  void IncrementNumber(int offset = 1);
  void SetNumber(int val) { mNumber = val; }

  bool operator== (const Modifier& that) const
  {
    return mType==that.mType && mNumber==that.mNumber && ConstStringPtr::operator ==(that);
  }

  bool operator< (const Modifier& that) const
  {
    if(mType  !=that.mType)   return mType  <that.mType;
    if(mNumber!=that.mNumber) return mNumber<that.mNumber;
    return ConstStringPtr::operator<(that);
  }
  Modifier& operator= (const Modifier& that)
  {
    mType = that.mType;
    mNumber = that.mNumber;
    ConstStringPtr::operator =(that);
    return *this;
  }
  bool operator!= (const Modifier& that) const
  {
...
typedef std::set<Modifier> ModifierSet;

AST::ModifierSet IncrementModifierSet(const ModifierSet& src);



}

#endif // AMD_ASTMODIFIER_HPP
