...
#ifndef AMD_PARSERVARIANT_HPP
#define AMD_PARSERVARIANT_HPP
...
/**
 * The datatype used by the parser to construct the tree                                                                    
 */
class ParserVariant : PositionAware, SP_LEAKDETECT_BASE
{
public:
  enum Type { T_UNSET, T_STRING, T_TREE_ELEMENTS, T_MODIFIER };

  static ParserVariant EmptyList() { return ParserVariant(T_TREE_ELEMENTS); }

  ///
  /// Null constructor
  ParserVariant(Type t = T_UNSET);

  ///
  /// String constructor
  ParserVariant(const char* str);
  ParserVariant(const std::string& str);
  ParserVariant(const ConstStringPtr& str);


  ///
  /// Tree Element constructor (start with one element
  ParserVariant(const AST::TreeElementPtr& elmt);

  ///
  /// Tree Element constructor (start with one element
  ParserVariant(AST::TreeElement* elmt);

  ///
  /// Modifier Constructor
  ParserVariant(const AST::Modifier& mod);

  ///
  /// Automatic conversion to string
  operator const std::string& () const;
  operator const ConstStringPtr& () const;
  
  const std::string& Str() const { return *this; }
  const AST::TreeElementPtr& Elmt() const { return *this; }

  operator const AST::Modifier& () const { ASSERT_ERROR(mType==T_MODIFIER)(ToString()); return mModifier; }

  ///
  /// Automatic conversion to List
  operator const AST::TreeElementList& () const;

  ///
  /// Automatic conversion to single Element
  operator const AST::TreeElementPtr& () const;

  ///
  /// Append an element to list type
  void Append(const AST::TreeElementPtr& elmt);

  ///
  /// Generic combination operator, merges two lists
  /// Only valid for T_TREE_ELEMENTS
  friend ParserVariant operator+ (const ParserVariant& a, const ParserVariant& b);

  ///
  /// Add a list of child nodes to this;
  void AddChildren(const ParserVariant& childList);

  bool IsEmpty() const { return Size()==0; }
  int Size() const { return (int)mElements.size(); }
  int NumChildren() const { return operator const AST::TreeElementPtr&()->NumChildren(); }


  std::string ToString() const;

  friend std::ostream& operator<< ( std::ostream& o, const ParserVariant& pv)
  {
    return o << pv.ToString();
  }
private:
  Type mType;
  ConstStringPtr mStr;
  AST::TreeElementList mElements;
  AST::Modifier mModifier;
};
