...
#ifndef AMD_ASTUSERDATATYPE_HPP
#define AMD_ASTUSERDATATYPE_HPP
...
{
public:
  UserDataType(const ConstStringPtr& name)
    : DataType(name)
  {}
};
 ...
 */
class StructDataType : public UserDataType
{
protected:
  friend class StructDeclaration;

  ///
  /// Only StructDeclaration is allowed to construct us
  StructDataType( const ConstStringPtr& name, SymbolTable members ) 
    : UserDataType(name) , mMembers(members)
  {}
