...
#ifndef AMD_ASTDELAYEDLOOKUP_HPP
#define AMD_ASTDELAYEDLOOKUP_HPP
...
/**
 * Contains a symbol that should be looked up once the symbol tables are built                                                                    
 */
class DelayedLookup : public TreeElement
{
  DCL_ASTELEMENT_METHODS(DelayedLookup);
public:
  DelayedLookup(const ConstStringPtr& name)
    : mName(name)
  {}

  DelayedLookup(const SymbolPtr& symbol)
    : mName(symbol->Name())
  {
  ...
  int ResolveSymbols(TreeElementPtr& self, SymbolTable*& scope);
  ...
  AST::DelayedLookup& AsDelayedLookup();

  bool IsDataTypeSymbol() const { return mDataTypeValue != 0; }
  const DataTypePtr& DataTypeValue() const { return mDataTypeValue; }

private:
  ConstStringPtr mName;
  DataTypePtr mDataTypeValue;
};


}

#endif // AMD_ASTDELAYEDLOOKUP_HPP
