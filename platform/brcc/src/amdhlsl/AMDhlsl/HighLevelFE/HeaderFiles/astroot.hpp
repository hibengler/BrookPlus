...
#ifndef AMD_ASTROOT_HPP
#define AMD_ASTROOT_HPP
...
{
  class Root;
  typedef SmartPtr<Root> RootPtr;

  class Root : public DataTypeScope
  {
    DCL_ASTELEMENT_METHODS(Root);
  public:
    ///
    ///mutable singleton instance
    static RootPtr& TheCurrentTreeRoot();
    ...
    ConstShaderDescriptionPtr CompileShaderToStream(std::ostream& os);

    // assorted information carried over the compile goes here
    // 
    // dx9 :register declarations can force a particular sampler to be used
    // each bit in dx9_samplers is set when the app forced use of the sampler id
    int dx9_samplers;
    void RecordForcedSampler(int samplerid) 
      {
        dx9_samplers |= 1<< samplerid;
      }
    Root(): dx9_samplers(0) {}
  };
