...
#ifndef AMD_ILTOKENTRANSCRIBER_HPP
#define AMD_ILTOKENTRANSCRIBER_HPP

typedef unsigned int TOKEN_TYPE;

#include "iltranscriber.hpp"

class ILTokenTranscriber : public ILTranscriber
{
public:


  ILTokenTranscriber(std::ostream& t) : ILTranscriber(t) {}

  virtual ILRegister AllocArrayStructure(int size, ILSpec::ILRegType type, int regNum);
  virtual void DclLiteral( ILRegister lit, long long unsigned int x, RegDT::RegDT dt );
  virtual void DclInput(ILSpec::ILImportUsage usg, ILSpec::ILInterpMode interp, ILRegister reg);
  virtual void DclOutput(ILSpec::ILImportUsage usg, ILRegister reg);
...
protected:
  ...
  virtual void op_3i_1o(const char* txt , ILSpec::ILOpCode bin, const ILRegister& d, const ILRegister& a, const ILRegister& b, const ILRegister& c);
  ///Two input one output format
  virtual void op_2i_1o(const char* txt , ILSpec::ILOpCode bin, const ILRegister& d, const ILRegister& l, const ILRegister& r, int);
  ///One input one output format
  virtual void op_1i_1o(const char* txt , ILSpec::ILOpCode bin ,ILRegister d, ILRegister l, int control);
  ///One input zero output format
  virtual void op_1i_0o(const char* txt , ILSpec::ILOpCode bin ,ILRegister d);
  ///Zero in/out format
  virtual void op_0i_0o(const char* txt , ILSpec::ILOpCode bin, int control);
  ///output il header
  virtual void op_il_hdr(const char* txt, ILSpec::IL_Shader_Type t);
  ///0i_0o op followed by an int
  virtual void op_with_int(const char* txt, ILSpec::ILOpCode bin, unsigned int x);
  ...
  virtual void op_resource(const char* txt , ILSpec::ILOpCode bin ,
                           ILRegister d, int resource,
                           int sampler,
                           TextureLoadFlags::TextureLoadFlags flags,
                           unsigned int offset,
                           ILRegister a0, ILRegister a1, ILRegister a2);
  
  virtual void DclLdsSizeOrMode(ILSpec::ILOpCode bin, int n, ILSpec::IL_LDS_SHARING_MODE m);  

  virtual void writeToken(TOKEN_TYPE t) = 0;
  void writeDstToken(const ILRegister& rg);
  void writeSrcToken(const ILRegister& rg);
};


class ILBinaryTokenTranscriber : public ILTokenTranscriber
{
public:
  ILBinaryTokenTranscriber(std::ostream& t) : ILTokenTranscriber(t) {}
protected:
  virtual void writeToken(TOKEN_TYPE t);
};

class ILHexTokenTranscriber : public ILTokenTranscriber
{
public:
  ILHexTokenTranscriber(std::ostream& t) : ILTokenTranscriber(t) {}
protected:
  virtual void writeToken(TOKEN_TYPE t);
};

#endif // AMD_ILTOKENTRANSCRIBER_HPP
