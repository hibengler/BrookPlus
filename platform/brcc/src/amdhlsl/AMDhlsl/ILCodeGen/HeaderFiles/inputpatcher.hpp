...
#ifndef AMD_INPUTPATCHER_HPP
#define AMD_INPUTPATCHER_HPP
...
//#define DISABLE_INPUTPATCHER

#ifdef DISABLE_INPUTPATCHER
#define inputPatcherApplyOverrideTable(tbl,name,suggested, offset) Override(suggested, "")
#endif
...
/**
 * Designed to patch the input of a hlfrontend program such that the inputs
 * occupy the same registers as the input's of FXC.  This is used only for
 * debugging and should be disabled in production systems.
 */
class InputPatcher
{  
public:
  struct Override 
  {
    Override():number(-1){memset(slots,0,sizeof(slots));}
    Override(int n, const std::string& s);
    int number;
    char slots[4];
  };
  typedef std::map< AST::Modifier, Override > OverrideTable;

  Override PatchResourceRegNum(const AST::Modifier& name, int suggested)
  {
    return inputPatcherApplyOverrideTable(mConstBufOrResourceOverrides,name,suggested,mConstBufMax);
  }
  Override PatchInputRegNum(const AST::Modifier& name, int suggested)
  {
    return inputPatcherApplyOverrideTable(mInputOverrides,name,suggested,mInputMax);
  }
  Override PatchOutputRegNum(const AST::Modifier& name, int suggested)
  {
    return inputPatcherApplyOverrideTable(mOutputOverrides,name,suggested,mOutputMax);
  }

  void ParseFxcListing(std::istream& input);
  void ParseFxcListing(const std::string& filename)
  {
    std::ifstream file(filename.c_str());
    ASSERT_ERROR(file.is_open())(filename)
      .Text("failed to open file");
    ParseFxcListing(file);
  }

#ifndef DISABLE_INPUTPATCHER

  InputPatcher() : mConstBufMax(0), mInputMax(0), mOutputMax(0) {}

protected:
  static Override inputPatcherApplyOverrideTable(const OverrideTable& tbl, const AST::Modifier& name, int suggested, int offset);


private:
  OverrideTable mConstBufOrResourceOverrides;
  OverrideTable mInputOverrides;
  OverrideTable mOutputOverrides;
  int mConstBufMax;
  int mInputMax;
  int mOutputMax;
