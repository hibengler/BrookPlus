...
{
  enum RegDT
  { //the order here is the same order used for auto-"promotion" of registers
    Invalid = 0,
    Bool,
    UChar,
    Char,
...
class BasicILRegister
{
public:
  static const char DEFAULT_SWIZZLE[4];
  ...
  typedef ILSpec::ILRegType RegType;
  ///
  /// Constructor
  BasicILRegister( int num = -1, RegType type = ILSpec::IL_REGTYPE_TEMP, RegDT::RegDT dt = RegDT::Invalid, int slots = -1)
    : mNumber(num), mType(type), mDT(dt)
  {
    SetNumSlots(slots);
  }

  virtual ~BasicILRegister(){}
  ...
  bool operator==(const BasicILRegister& that)
  {
    return that.mNumber==mNumber && that.mType == mType;
  }
  bool operator!=(const BasicILRegister& that)
  {
  ...
  friend std::ostream& operator << ( std::ostream& os, const BasicILRegister& r )
  {
    return os << r.ToString();
  }

  //Datatype
  RegDT::RegDT GetDT() const { return mDT; }
  void SetDT(RegDT::RegDT val) { mDT = val; }

  int GetNumSlots() const;
  void SetNumSlots(int val);

  std::string SwizzleString(bool isDst) const;
  const char* SwizzleBytes() const { return mSwizzle; };
  void SetSwizzleBytes(const char v[4]);
  bool IsStandardSwizzle() const;
  void ApplyDstSwizzle(const char v[4]);

  //read members
  int Number() const { return mNumber; }
  BasicILRegister::RegType Type() const { return mType; }

  BasicILRegister(const BasicILRegister& that)
    {
      mNumber = that.mNumber;
      mType = that.mType;
      mDT = that.mDT;
      memcpy(&mSwizzle, &(that.mSwizzle), sizeof(mSwizzle));
    }
...
class ILRegister : public BasicILRegister
{
public:
  enum 
  { 
  ...
  ILRegister( int num = -1, RegType type = ILSpec::IL_REGTYPE_TEMP, RegDT::RegDT dt = RegDT::Invalid, int slots = -1)
    : BasicILRegister( num, type, dt, slots)
    , mOffset(0)
    , mFlags(0)
  {}

  ILRegister(const BasicILRegister& that)
    : BasicILRegister(that)
    , mOffset(0)
    , mFlags(0)
  {}

  ILRegister(const ILRegister& that)
    : BasicILRegister(that)
  ...
  std::string DstILString() const
  {
    return ToString() + SwizzleString(true);
  }

  std::string SrcILString() const
  {
    return ToString() + FlagString() + SwizzleString(false);
  }

  int GetOffset() const { return mOffset; }
  void SetOffset(int val) { mOffset = val; }
  void IncrementOffset(int n) { SetOffset(GetOffset()+n); }

  BasicILRegister GetBase() const { return mBase; }
  void SetBase(BasicILRegister val) { mBase = val; }

  int GetFlags() const { return mFlags; }
  void SetFlags(int val) { mFlags = val; }
  void AddFlags(int val) { mFlags |= val; }

  std::string FlagString() const;

  void WidenSwizzleTo(int slots);


  bool IsArrayType() const { return mBase.operator bool(); }

private:
  //for indexed temps
  int mOffset;
  BasicILRegister mBase;
  int mFlags;

};

#endif // AMD_ILREGISTER_HPP
