...
#ifndef AMD_ILTRANSLATOR_HPP
#define AMD_ILTRANSLATOR_HPP
...
namespace AST { class BasicSymbol; }
...
public:
  ///
  /// Constructor
  ILTranslator( ILTranscriber* _il );

  ///
  /// Convert a pair of operation and datatype to IL commands
  void BinaryCommand(OpType::OpType op, ILRegister& dst, ILRegister left, ILRegister right ) const;

  ///
  /// Convert a pair of operation and datatype to IL commands
  void UnaryCommand(OpType::OpType op, ILRegister& dst, ILRegister src ) const;
 
  ///
  /// Convert a register from one type to another
  void ConvertTo(RegDT::RegDT dstdt, ILRegister& dst, ILRegister src ) const;

  static int RunOpStatically(OpType::OpType op, int l, int r);
  static int RunUnaryOpStatically(OpType::OpType op, int l);  

  ///
  /// Allocate a temporary register and convert src to newdt into it
  ILRegister TmpConvertInput(RegDT::RegDT newdt, ILRegister src) const;

  ///
  /// Resize a register to a different number of slots.  Filling it zeros where needed.
  ILRegister TmpReslotInput( int slots, ILRegister src ) const;

public:

  ///
  ///Convert inputs to a new type and retry
  void retryBinaryCommandAs(RegDT::RegDT newdt, OpType::OpType op, ILRegister& dst, ILRegister left, ILRegister right ) const;

  ///
  /// Allocate a temp containing negated input
  ILRegister tmpNegate(ILRegister src) const;


  ///
  /// We put large hacks in handleSpecialCases, so as to not clutter the table
  void handleSpecialCases(OpType::OpType op, ILRegister& dst, ILRegister left, ILRegister right ) const;

private:
  ILTranscriber*  il;
};

#endif // AMD_ILTRANSLATOR_HPP
