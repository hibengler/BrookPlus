...
#ifndef AMD_TIMER_HPP
#define AMD_TIMER_HPP
...
public:
  ...
  static void PrintTimerReport(std::ostream& os = std::cerr);
private:
  double mStartedAt;
  double mLastTime;
  double mTotalTime;
...
{
public:
  ScopeTimer(StopWatch& sw) : mTimer(sw) { mTimer.Start(); }
  ~ScopeTimer() { mTimer.Stop(); }
...
#define SCOPE_TIMER(name) static StopWatch _MK_UNIQUE_NAME(_timer)(name); ScopeTimer _MK_UNIQUE_NAME(_scope)(_MK_UNIQUE_NAME(_timer));
#define BEGIN_TIMER(name) { SCOPE_TIMER(name) 
#define END_TIMER }
#else
