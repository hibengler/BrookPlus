...
#ifndef AMD_HIERARCHICALLOOKUPTABLE_HPP
#define AMD_HIERARCHICALLOOKUPTABLE_HPP

#include "assertlogging.hpp"

//note, consider switching to ext/hash_map
#include <map>
#include <list>

/**
 * Forms a "stack" of lookup tables that should be searched in top to 
 * bottom order to find a entry
 */
template<typename Key, typename Val>
class HierarchicalLookupTable
{
  typedef std::pair<Key, Val> TPair;
  typedef std::list<TPair>    TList;//for order
  typedef std::map<Key, Val>  TMap; //for O(lgn) lookups
public:
  ///constructor
  HierarchicalLookupTable( const HierarchicalLookupTable* parent = NULL
                         , const Val& nullVal = Val() )
    : mParent(parent)
    , mNullValue(nullVal)
  {}

  ///lookup an element either here, or in a parent table
  const Val& Lookup( const Key& key) const
  {
    typename TMap::const_iterator i = mTable.find( key );

    if(i!=mTable.end())     return i->second; //Found it here
    else if(mParent!=NULL)  return mParent->Lookup( key );//Maybe parent has it?
    else                    return mNullValue; //Dont have it...
  }

  ///true if a given key is set
  bool HasKey( const Key& key ) const
  {
    return Lookup(key) != mNullValue;
  }

  ///true if a given key is set, at this level in table
  bool HasKeyLocally( const Key& key ) const
  {
    return mTable.find(key) != mTable.end();
  }

  ///insert a new value into this level of table
  void Insert(const Key& key, const Val& val)
  {
    ASSERT_ERROR(!HasKeyLocally(key))(key).Text("Element already in table");
    mList.push_back(TPair(key,val));
    mTable[key] = val;
  }
  ...
  size_t Size() const 
  {
    return mTable.size() + (mParent!=NULL ? mParent->Size() : 0);
  }

  ///
  /// Set the parent link
  void SetParent(const HierarchicalLookupTable* parent)
  {
    mParent = parent;
  }

  void Clear() { mTable.clear(); mList.clear(); mParent=0; }

  
  typedef typename TList::const_iterator ConstIterator;
  typedef typename HierarchicalLookupTable::ConstIterator Iterator; 
  ConstIterator Begin() const { return mList.begin(); }
  ConstIterator End() const   { return mList.end(); }
private:
  const HierarchicalLookupTable* mParent;
  const Val                      mNullValue;
  TMap                           mTable; //O(lgn) lookup
  TList                          mList;  //maintains order
};
