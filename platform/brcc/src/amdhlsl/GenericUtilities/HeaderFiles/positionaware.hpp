...
#ifndef AMD_POSITIONAWARE_HPP
#define AMD_POSITIONAWARE_HPP

#include <iostream>
#include "conststring.hpp"

/**
 * Represents functionality of "remembering where i came from" in some parsed
 * user file.  Allows printing of helpful debug messages to the user
 */
class PositionAware
{
public:
  static ConstStringPtr TheCurrentFile;
  PositionAware(int l = -1, ConstStringPtr f = TheCurrentFile);

  const ConstStringPtr& GetFile() const { return file; }
  void SetFile(const ConstStringPtr& val) { file = val; }
  int GetLine() const { return line; }
  void SetLine(int val) { line = val; }
  void SetPosition(const PositionAware& that)
  {
    SetFile(that.GetFile());
    SetLine(that.GetLine());
  }
  const PositionAware& GetPosition() const;
  bool IsPositionValid() const;
  friend std::ostream& operator<< (std::ostream& o, const PositionAware& pos );
  bool operator == ( const PositionAware& pos ) { return line==pos.line && file==pos.file; }
  bool operator != ( const PositionAware& pos ) { return ! operator==(pos); }
private:
  int line;
  ConstStringPtr file;
};

//this is the fallback version in case of assertion in a non-position aware class
const PositionAware& GetPosition();
void SetGlobalPosition(const PositionAware& pos);
void IncrementGlobalLineno();

#endif // AMD_POSITIONAWARE_HPP
