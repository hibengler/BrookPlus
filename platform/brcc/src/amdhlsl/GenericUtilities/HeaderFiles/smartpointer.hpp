...
#ifndef AMD_SMARTPOINTER_HPP
#define AMD_SMARTPOINTER_HPP

#include "assertlogging.hpp"
#include <list>


//#define SMARTPOINTER_LEAKDETECT

#ifdef SMARTPOINTER_LEAKDETECT
void SP_LEAKDETECT_RESET();
void SP_LEAKDETECT_REPORT();
class SP_LEAKDETECT_BASE 
{ 
public: 
  SP_LEAKDETECT_BASE();
  ~SP_LEAKDETECT_BASE();
...
#else
#define SP_LEAKDETECT_RESET()
#define SP_LEAKDETECT_REPORT()
class SP_LEAKDETECT_BASE {};
#endif

/**
 * Base class for all simple RefCounted objects
 */
class RefCountedObj : SP_LEAKDETECT_BASE
{
public:
  ...
  /// Default to 0 refs
  RefCountedObj() : mRefCount(0) {}

  /// Increment mRefCount
  void IncRefCount() const;

  /// Decrement mRefCount and delete self if reach 0
...
/**
 * Maintains a ref counted pointer to a given RefCountedObj
 */
template < typename T >
class SmartPtr 
{
public:
  ...
  SmartPtr()      : mObj(0) {}
  SmartPtr(T* p)  : mObj(p) { if(IsValid()) mObj->IncRefCount(); }
  SmartPtr(const SmartPtr& that) 
    : mObj(that.mObj) 
  { 
    if(IsValid()) mObj->IncRefCount(); 
  }
  ~SmartPtr()     { if(IsValid()) mObj->DecRefCount(); }
  ...
  T& operator* () const
  {
    DEBUG_CHECK(IsValid()).Text("Null pointer exception");
    return *mObj; 
  }
  ...
  SmartPtr& operator= (const SmartPtr& that)
  {
    T* old = mObj;
    mObj = that.mObj;
    //Must increment before decrement to handle self assignment correctly
    if(IsValid()) mObj->IncRefCount();
    if(old!=NULL) old->DecRefCount();
    return *this;
  }

  T* AsPtr() const { return mObj; } 

  //comparison operators
  bool operator==(const SmartPtr& that) const { return mObj == that.mObj; }
  bool operator!=(const SmartPtr& that) const { return mObj != that.mObj; }
  bool operator<(const SmartPtr& that) const { return mObj < that.mObj; }
private:
  T* mObj;
