...
#ifndef AMD_CONSTSTRING_HPP
#define AMD_CONSTSTRING_HPP
...
class ConstString;
typedef SmartPtr<const ConstString> ConstStringPtr;

/**
 * Copying strings around can get slow, this allows a central pool of heavily
 * used strings.  Can compare by pointers for fast equality tests.
 */
class ConstString : public std::string, public RefCountedObj
{
private:
  ConstString(const std::string& str) : std::string(str) {}
  ConstString(const ConstString&);
public:
  static ConstStringPtr Lookup(const std::string& str);
  ~ConstString();
};

inline std::ostream& operator << ( std::ostream& o, const ConstStringPtr& str)
{
  if(str.IsValid())
    return o << *str;
  else
...
#define STATIC_CONSTSTRING(name, str) static ConstStringPtr name = ConstString::Lookup(str)

#endif // AMD_CONSTSTRING_HPP
