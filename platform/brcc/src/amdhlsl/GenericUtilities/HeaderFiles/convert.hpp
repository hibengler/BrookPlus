...
#ifndef _AMD_CONVERT_H
#define _AMD_CONVERT_H

#include<string>
#include<sstream>

/**
 * Generic FromString() functions based on stl string stream
 */
template<typename To> To FromString (const std::string& value)
{
  To ret;
...
/**
 * Generic ToString() functions based on stl string stream
 */
...
#define _COMPILETIME_STRINGIFY(x) #x 
#define COMPILETIME_STRINGIFY(x) _COMPILETIME_STRINGIFY(x)
...
inline std::string ToLower(std::string str)
{
  for(size_t i=0; i<str.size(); ++i)
    if(Between('A',str[i],'Z'))
      str[i] += 'a'-'A'; //shift in ascii table
  return str;
}
...
template <typename T>
inline T Minimum(const T& a, const T& b) { return a<=b ? a : b; }

template <typename T>
inline T Maximum(const T& a, const T& b) { return a>=b ? a : b; }


///true if haystack starts with needle
inline bool StartsWith(const char* haystack, const char* needle)
{
  while(*haystack==*needle && *needle!=0) ++haystack, ++needle;
  return *needle==0;
}

inline bool StartsWith(const std::string& haystack, const char* needle)
{
  return StartsWith(haystack.c_str(),needle);
}
