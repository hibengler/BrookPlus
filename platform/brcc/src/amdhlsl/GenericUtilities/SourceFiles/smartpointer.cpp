...
/// Increment mRefCount
void 
RefCountedObj::IncRefCount() const 
{ 
  ATOMIC_INCREMENT(mRefCount);
}

/// Decrement mRefCount and delete self if reach 0
void 
RefCountedObj::DecRefCount() const
{ 
  DEBUG_CHECK(mRefCount>0);
  if( ATOMIC_DECREMENT(mRefCount) == 0)
  {
    delete this;     
  }
}
...
static std::set<const SP_LEAKDETECT_BASE*> theAllocatedObjects;
void SP_LEAKDETECT_RESET()
{
  theAllocatedObjects.clear();
};
void SP_LEAKDETECT_REPORT()
{
  std::cerr << theAllocatedObjects.size() << " leaked objects" << std::endl;
}
SP_LEAKDETECT_BASE::SP_LEAKDETECT_BASE()
{
  theAllocatedObjects.insert(this);
}

SP_LEAKDETECT_BASE::~SP_LEAKDETECT_BASE()
{
  theAllocatedObjects.erase(this);
}
#endif
