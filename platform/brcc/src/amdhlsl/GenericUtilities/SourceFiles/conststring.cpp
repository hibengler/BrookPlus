...
namespace
{
  //we will actually hold two copies of the string in this case... not ideal
  //but for things like keywords will still save time as it scales up
  std::map<std::string, ConstString*>&  theTable()
  {
    //we just leak this this one object as we dont want it deleted ever
    static std::map<std::string, ConstString*>* inst = new std::map<std::string, ConstString*>();
    return *inst;
  }
}


ConstStringPtr ConstString::Lookup( const std::string& str )
{
  std::map<std::string, ConstString*>::iterator entry = theTable().find(str);
  if(entry==theTable().end())
    return theTable()[str]=new ConstString(str);
  else
    return entry->second;
}
