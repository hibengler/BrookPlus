...
PositionAware::PositionAware( int l /*= ::line*/, ConstStringPtr f /*= ::file*/ ) : line(l), file(f)
{

}

const PositionAware& PositionAware::GetPosition() const
{
  return IsPositionValid() ? (*this) : (::GetPosition());
}

bool PositionAware::IsPositionValid() const
{
  return line>0 && file.IsValid();
}

static PositionAware theCurPos;
...
{
  theCurPos.SetLine(theCurPos.GetLine()+1);
}

std::ostream& operator<< (std::ostream& o, const PositionAware& pos )
{
...
ConstStringPtr PositionAware::TheCurrentFile(0);
