...
#ifdef DEBUG
PARAMETER(p_debug, "debug");
PARAMETER_DESC("debug", "Causes program to break into debugger on error");
#else
...
{
  //turn /a/b/c/d into d
  const char* simple_basename(const char* dirpath)
  {
    for(const char* i=dirpath; *i!=0; ++i)
      if(*i=='/' || *i=='\\')
        dirpath=i+1;
    return dirpath;
  }

  static std::ostream* theOutputStream = &std::cerr;
#define LOG (*theOutputStream)
#define ENDL ('\n') //this is faster than std::endl that autoflushes
}

void AssertLoggingSetOutputStream( std::ostream* s )
{
  if(s==NULL) s=&std::cerr;
  theOutputStream->flush();
  theOutputStream = s;
}

const assert_logging_internal::DebugInfoEnd _CONTINUE_ASSERT_BLOCK_A;
const assert_logging_internal::DebugInfoEnd _CONTINUE_ASSERT_BLOCK_B;

assert_logging_internal::DebugInfoContext::DebugInfoContext( const char* file
                                                           , int line
                                                           , const char* func)
  : mFilename(file)
  , mLine(line)
...
std::ostream&
assert_logging_internal::operator<< (std::ostream& os, const DebugInfoContext& v)
{
  os << v.mFunction << "()[" << simple_basename(v.mFilename) << ':' << v.mLine << ']';
  return os;
}

std::ostream&
assert_logging_internal::operator<< (std::ostream& os, const DebugInfoEnd& v)
{
  os << "Message: " << v.mErrorText;
  return os;
}

std::ostream& 
assert_logging_internal::operator<< (std::ostream& os, const DebugInfoVariable& v)
{
  os << v.mName << " = " << v.mValue;
  return os;
}

assert_logging_internal::DebugInfoReciever::DebugInfoReciever(bool isFatal, const char* des)
    : mIsFatal(isFatal)
    , mDescription(des)
{
  LOG << mDescription << ENDL;
}

assert_logging_internal::DebugInfoReciever& 
assert_logging_internal::DebugInfoReciever::operator<< (const DebugInfoContext& ctx)
{
  LOG << "In compiler at " << ctx << ENDL;
  return *this;
}

assert_logging_internal::DebugInfoReciever& 
assert_logging_internal::DebugInfoReciever::operator<< (const PositionAware& pos)
{
  if(pos.IsPositionValid())
  {
    LOG << "While processing " << pos << ENDL;
  }
  return *this;
}

assert_logging_internal::DebugInfoReciever& 
assert_logging_internal::DebugInfoReciever::operator<< (const DebugInfoVariable& var)
{
  LOG << "  " << var << ENDL;
  return *this;
}
...
assert_logging_internal::DebugInfoReciever& 
assert_logging_internal::DebugInfoReciever::operator<< (const DebugInfoEnd& endinfo)
{
  if(endinfo.HasErrorText())
    LOG << endinfo << ENDL;
  ...
  if(mIsFatal)
  {
    ...
    if(p_debug) BREAK_INTO_DEBUGGER;

#if CONFIG_ASSERT_USE_EXCEPTIONS
    throw 1;
#else
    exit(1); //this is much faster than a throw
#endif
  }
...
assert_logging_internal::DebugInfoEnd::DebugInfoEnd(const std::string& text)
  : mErrorText(text)
{}
bool assert_logging_internal::DebugInfoEnd::HasErrorText() const 
{ 
  return mErrorText != ""; 
}
assert_logging_internal::DebugInfoEnd assert_logging_internal::DebugInfoEnd::Text(const std::string& text) const 
{ 
  return text; 
}
