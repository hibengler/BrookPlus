...
Register::Register(string reg, string mask, string negate):string(reg), swizzle(mask), negate(negate)
{
}

DefineConstantRegister::DefineConstantRegister(const Register & reg,
                                               float a, float b, float c, float d):Declaration(reg)
{
    Symbol s;
...
bool IntermediateLanguage::makeTemporaries(std::vector < Symbol > &ret, int num, int maxtemporaries)
{
...
void Symbol::Specify(const Symbol & sym)
{
    ...
    {
        properties.x = sym.properties.x;
        properties.y = sym.properties.y;
        properties.z = sym.properties.z;
        properties.w = sym.properties.w;
...
Symbol::Symbol()
{
    ...
    lineno = -1;
    properties.x = properties.y = properties.z = properties.w = -999;
}
...
void Symbol::Set(TYPE type, unsigned int UNUSED(index), float x, float y, float z, float w)
{
    properties.validfloats = true;
    properties.x = x;
    properties.y = y;
