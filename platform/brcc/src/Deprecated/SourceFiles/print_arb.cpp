...
static std::ostream & printTexldX(std::ostream & s,
                                  OpCode op,
                                  const Register & dst,
                                  const Register & src0, const Register & src1, const char *UNUSED(default_tex) = "2D")
{
    op.print_arbfp(s);
    dst.print_arbfp(s) << arbdelim;
    src0.print_arbfp(s) << arbdelim;
...
std::ostream & PowOp::print_arbfp(std::ostream & s) const
{
    ...
    exp.opcode = ExpOp_str;
    OpCode mul(op);

    mul.opcode = MulOp_str;
    OpCode log(op);

    log.opcode = LogOp_str;
    log.print_arbfp(s);
    dst.print_arbfp(s) << arbdelim;
    ...
    if (1)
    {
        ...
        for (map < string, Symbol >::iterator i = symbolTable.begin(); i != symbolTable.end(); ++i)
        {
            ...
            case Symbol::CONST:
                ...
                {
                    s << "PARAM " << nam << " = {" << sym->properties.x << ", " << sym->properties.y << ", " << sym->
                        properties.z << ", " << sym->properties.w << "}" << arbendl;
                }
                else
                {
