...
char *compile_fxc(const char *name,
                  const char *shader, CodeGenTarget target, ShaderResourceUsage * outUsage, bool inValidate)
{
    LOG(LOG_FN_ENTRY, "compile_fxc(%s, %s, %d, %p, %d)\n", name, shader, target, outUsage, inValidate);

    static const int kInputFileArgument = 5;
    static const int kOutputFileArgument = 4;
    const char validate[] = "/Vd";

    //char software[]="/Tps_2_sw";
    const char hardware[] = "/Tps_2_0";
    const char ps2b[] = "/Tps_2_b";
    const char ps2a[] = "/Tps_2_a";
    const char ps30_targetstring[] = "/Tps_3_0";
    const char nothin[] = "";         //gcc does not like ?: with ""
    ...
    if (errcode == NULL)
    {
        fprintf(stderr, "%s resulted in an error," "skipping ", argv[0]);
    ...
    while ((i = fgetc(fp)) != EOF)
    {
        ...
        else if (pos > 0 && fpcode[pos - 1] == '/' && i == (int) '/')
        {
            incomment = true;
            comments[cpos++] = '/';
            comments[cpos++] = '/';
