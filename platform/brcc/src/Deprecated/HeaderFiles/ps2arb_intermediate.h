   ...
   virtual std::ostream & print_arbfp(std::ostream & s) const {return s;}
    ...
    class OpCode
    {
      public:
        ...
        string opcode;
               OpCode(string opcode, const InstructionFlags & flags):flags(flags), opcode(opcode)
        {
        }
        virtual ~ OpCode()
        {
    ...
    class Instruction:public Statement
    {
      public:
        ...
        void SetLowPrecision(InstructionFlags::ACCURACY a)
        {
            op.flags.acc = a;
        }
        void SetSaturated(bool sat)
        {
            op.flags.saturate = sat ? 1 : 0;
        }
        PRINT_FUNCTIONS;
    ...
    class UnaOp:public Instruction
    {
      public:
        ...
        UnaOp(const OpCode & instruction, const Register & dst, const Register & src0):Instruction(instruction),
            dst(dst), src0(src0)
        {
        }
        PRINT_PROTOTYPES;
    ...
    class BinOp:public Instruction
    {
      public:
        ...
        BinOp(const OpCode & instruction, const Register & dst, const Register & src0,
              const Register & src1):Instruction(instruction), dst(dst), src0(src0), src1(src1)
        {
        }
        PRINT_PROTOTYPES;
    ...
    class TriOp:public Instruction
    {
      public:
        ...
        TriOp(const OpCode & instruction, const Register & dst, const Register & src0, const Register & src1,
              const Register & src2):Instruction(instruction), dst(dst), src0(src0), src1(src1), src2(src2)
        {
        }
        PRINT_PROTOTYPES;
    ...
    typedef BinOp *(BinaryFactory) (const InstructionFlags & iflag, const Register & dst, const Register & src0,
                                    const Register & src1);
    typedef TriOp *(TrinaryFactory) (const InstructionFlags & iflag, const Register & dst, const Register & src0,
...
#undef XXX
    ...
    class Symbol
    {
      public:
        ...
        enum TEXTARGET
        { UNSPECIFIED, TEX1D, TEX2D, TEX3D, CUBE, RECT };
        struct Properties
        {
        ...
        {
            return type == a.type && registerindex == a.registerindex
                && properties.texturetarget == a.properties.texturetarget;
        }
        Symbol();
