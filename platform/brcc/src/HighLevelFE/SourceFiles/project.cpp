...
extern void SetBrccInputStream(std::istream* input);
...
#ifdef  DEBUG_CEA
    std::cerr << std::endl << "Std include path[0]: \"" << StdPath[0] << "\"" << std::endl;
#endif
            ...
            if (stemnt->isInclude())
            {
...
#endif
                {
                    ...
                    *inclEnd = '\0';
                    StdPath[1] = inclPath;
#ifdef  DEBUG_CEA
                    std::cerr << "Std include path[1]: \"" << StdPath[1] << "\"" << std::endl;
#endif
                }
                else
                    delete[]inclPath;
...
TransUnit::TransUnit(const std::string & fname):filename(fname)
{
    head = tail = NULL;
}
...
static void TransformBlockStemnt(Block * fd, fnStemntTransformer cb)
{
    if (!fd)
        return;
...
std::ostream & operator<<(std::ostream & out, const TransUnit & tu)
{
    int inInclude = 0;
    Statement *stemnt;
...
TransUnit *Project::parse(const char *path, bool use_cpp,       /* =true */
...
*/
    ...
    if (!fp)
    {
        std::cerr << "Unable to open \"" << cpp_file << "\" file.\n";
        return NULL;
    }
    ...
    {
        SetBrccInputStream(Parse_TOS->yyinstream);
    }
