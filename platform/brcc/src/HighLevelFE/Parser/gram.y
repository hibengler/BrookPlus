...
/* ###################################################### */
%}
/* FIXME - not supported by cygwin version of bison %error-verbose */
/* two shift/reduce conflicts regarding the error token */
%expect 2
...
%token <consValue>  STRING LSTRING ASM
%token <consValue>  CHAR_CONST LCHAR_CONST
%token <consValue>  INUM RNUM

 /* Note: (FIXME) Handling preprocessor tokens inside the main grammar is a
  * really bad idea - given that pp stuff can appear between any two tokens
  * the resulting combined grammar is context-sensitive therefore not actually
  * parsable by yacc. For now, we hack in special cases when they show up, but
  * eventually we will need to provide a separate pp pass.
  */
...
/* the reserved words */
%token <typeQual>   CONST VOLATILE OUT SHARED REDUCE VOUT ITER KERNEL RESTRICT
%token <storage>    AUTO EXTRN REGISTR STATIC TYPEDEF
%token <base>       VOID CHAR CHAR2 CHAR3 CHAR4 SHORT SHORT2 SHORT3 SHORT4 INT INT2 INT3 INT4 LONG SGNED UNSGNED GCC_BUILTIN
/* IMPORTANT: Keep all the FLOATN's next to each other in order! */
%token <base>       FLOAT FLOAT2 FLOAT3 FLOAT4 DOUBLE DOUBLE2
...
%token <loc>        TARGET ASM_SCRATCH ASM_READ ASM_WRITE
...
%token              CONSTRUCTOR_FLOAT CONSTRUCTOR_DOUBLE CONSTRUCTOR_CHAR CONSTRUCTOR_UCHAR CONSTRUCTOR_SHORT CONSTRUCTOR_USHORT
%token              CONSTRUCTOR_INT CONSTRUCTOR_UINT 

/* Gcc Extensions */
%token              ATTRIBUTE KERNEL_ATTRIBUTE ALIGNED PACKED CDECL MODE FORMAT NORETURN NOTHROW
%token              NONNULL PURE GROUPSIZE UNKNOWN_GCC_ATTRIBUTE UNKNOWN_KERNEL_ATTRIBUTE MALLOC WARN_UNUSED_RESULT

/* Add precedence rules to solve dangling else s/r conflict */
// FIXME - factor this properly, don't need precedence rules here
%nonassoc IF
%nonassoc ELSE
...
%right              CAST
%right <loc>        UNARY NOT B_NOT SIZEOF INDEXOF INSTANCE INSTANCE_IN_GROUP SYNC_GROUP INCR DECR
%left               HYPERUNARY
...
%type  <value>       sizeof_expr indexof_expr instance_expr instance_in_group_expr sync_group_expr addr_expr indirection_expr
%type  <value>       preinc_expr predec_expr constructor_expr const_constructor_expr
...
%type  <stemnt>      if_stemnt if_else_stemnt asm_stemnt
...
%type  <declStemnt>  declaration_single
%type  <declStemnt>  declaration_list opt_declaration_list opt_declaration_list_actual
%type  <declStemnt>  opt_KnR_declaration_list
...
%type  <base>        constructor_op2 constructor_op3 constructor_op4
...
%type  <gccAttrib>   gcc_attrib opt_gcc_attrib gcc_attrib_single gcc_inner kernel_attrib_single kernel_inner

%type  <inumlist>    inum_list
...
func_def:          
        kernel_attrib_single func_spec cmpnd_stemnt
        {
            if ($3 != NULL)
            {
                $$ = new FunctionDef($3->location);
                Block *blk = (Block*) $3;
...
stemnt_list_reentrance2: stemnt_reentrance
        ...
        {    /* preprocessor #line directive */
          ...
          | stemnt_list_reentrance2 error SEMICOLON
        {
            $$ = $1;
        }
      ;
...
asm_stemnt: ASM
        {
#ifdef __ASMIL__
            $$ = new ASMStemnt($1, $1->location);;
#else
            $$ = NULL; /* We should never get here as asm supporte disabled in lexer. */
#endif /* __ASMIL__ */
        }
        ;
...
assign_expr:  cond_expr
        ...
        {
            $$ = new AssignExpr($2,$1,$3,currentLocation);
        }
...
equality_expr:  relational_expr
        ...
        {
            $$ = new RelExpr($2, $1, $3, currentLocation);
        }
        ;
...
shift_expr:  additive_expr
        ...
        {
            $$ = new BinaryExpr($2, $1, $3, currentLocation);
        }
        ;
...
mult_expr:  cast_expr
        ...
        ;
constructor_op2: FLOAT2 | INT2 | DOUBLE2 | CHAR2 | SHORT2;
constructor_op3: FLOAT3 | INT3 | CHAR3 | SHORT3;
constructor_op4: FLOAT4 | INT4 | CHAR4 | SHORT4;

constructor_expr: constructor_op2 LPAREN assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $3, $5 };
            $$ = new ConstructorExpr($1, exprs, currentLocation);
        }
                | constructor_op3 LPAREN assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $3, $5, $7 };
            $$ = new ConstructorExpr($1, exprs, currentLocation);
        }
                | constructor_op4 LPAREN assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $3, $5, $7, $9 };
            $$ = new ConstructorExpr($1, exprs, currentLocation);
        }
                | constructor_op2 LPAREN error RPAREN
        {
           $$ = (Expression *) NULL;
        }
                | constructor_op3 LPAREN error RPAREN
        {
           $$ = (Expression *) NULL;
        }
                | constructor_op4 LPAREN error RPAREN
        {
           $$ = (Expression *) NULL;
        }
        ;
const_constructor_expr: CONSTRUCTOR_FLOAT assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Float2);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_FLOAT assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            BaseType *bType = new BaseType(BT_Float3);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_FLOAT assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
            BaseType *bType = new BaseType(BT_Float4);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
        /* double constructors */
                | CONSTRUCTOR_DOUBLE assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
        ...
        /* int constructors */        
                | CONSTRUCTOR_INT assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Int2);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_INT assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_INT assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
        ...
        /* uint constructors */        
                | CONSTRUCTOR_UINT assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Int2 | BT_UnSigned);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_UINT assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_UINT assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
        ...
        /* short constructors */        
                | CONSTRUCTOR_SHORT assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Short2);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_SHORT assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_SHORT assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
        ...
        /* ushort constructors */        
                | CONSTRUCTOR_USHORT assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Short2 | BT_UnSigned);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_USHORT assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_USHORT assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
        ...
        /* char constructors */        
                | CONSTRUCTOR_CHAR assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Char2);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_CHAR assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_CHAR assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
        ...
        /* uchar constructors */        
                | CONSTRUCTOR_UCHAR assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4 };
            BaseType *bType = new BaseType(BT_Char2 | BT_UnSigned);
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
        }
                | CONSTRUCTOR_UCHAR assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_UCHAR assign_expr COMMA assign_expr COMMA
                                assign_expr COMMA assign_expr RPAREN
        {
            Expression *exprs[] = { $2, $4, $6, $8 };
            ...
            $$ = new ConstructorExpr(bType, exprs, currentLocation);
                ...
                | CONSTRUCTOR_FLOAT error RPAREN
        {
           $$ = (Expression *) NULL;
        }
         
                | CONSTRUCTOR_DOUBLE error RPAREN
        {
           $$ = (Expression *) NULL;
        }
         
                | CONSTRUCTOR_CHAR error RPAREN
        {
           $$ = (Expression *) NULL;
        }
         
                | CONSTRUCTOR_UCHAR error RPAREN
        {
           $$ = (Expression *) NULL;
        }
         
                | CONSTRUCTOR_SHORT error RPAREN
        {
           $$ = (Expression *) NULL;
        }
        
                | CONSTRUCTOR_USHORT error RPAREN
        {
           $$ = (Expression *) NULL;
        }
        
                | CONSTRUCTOR_INT error RPAREN
        {
           $$ = (Expression *) NULL;
        }
         
                | CONSTRUCTOR_UINT error RPAREN
        {
           $$ = (Expression *) NULL;
        }
        ;        
...
instance_expr: INSTANCE LPAREN RPAREN
        {
          $$ = new BrtInstanceExpr($1);
        }
        ;

instance_in_group_expr: INSTANCE_IN_GROUP LPAREN RPAREN
        {
          $$ = new BrtInstanceInGroupExpr($1);
        }
        ;
        
sync_group_expr: SYNC_GROUP LPAREN RPAREN
        {
          $$ = new BrtSyncGroupExpr($1);
        }
        ;        

unary_minus_expr:  MINUS cast_expr    %prec UNARY
        {
            $$ = new UnaryExpr(UO_Minus, $2, currentLocation);
        }
        ;
...
indirection_expr:  STAR cast_expr     %prec UNARY
        {
            $$ = new UnaryExpr(UO_Deref, $2, currentLocation);
        }
        ;
...
prim_expr:  ident
        {
            $$ = new Variable($1, currentLocation);
        }
         |  paren_expr
         |  constant
        {
...
opt_declaration_list:
 ...
 declaration_list:  declaration_single
        {
            $$ = $1;
        }
                |  declaration_list declaration_single
        {
            $$ = $1;
                        ...
                        Statement*      appendDecl = $1;
                        while (appendDecl->next != NULL)
                                appendDecl = appendDecl->next;

            appendDecl->next = $2;
        }
                | declaration_list error SEMICOLON
        {
            $$ = $1;
        }
        ;

declaration_single: declaration SEMICOLON
        {
            $$ = new DeclStemnt($2);
...
decl_specs_reentrance_bis: decl_specs_reentrance
        {
            if ($1->typemask == BT_Signed || $1->typemask == BT_UnSigned)
                $1->typemask |= BT_Int;
            $$ = $1;
            gProject->Parse_TOS->parseCtxt->SetDeclCtxt($$);
        }
...
decl_specs_reentrance:  storage_class opt_decl_specs_reentrance
          ...
          |  type_spec { possibleType = false; } opt_decl_specs_reentrance
        {
            ...
            {
                bool typeerr = false;
                if ($$->typemask == BT_Signed || $$->typemask == BT_UnSigned)
                {
                    $$->typemask |= $3->typemask;
                }
                ...
                if ($3->typemask & BT_Long)
                {
                    ...
                    else if ($$->typemask & BT_Int)
                    {
                        $$->typemask = $3->typemask;
                    }
                    else if ($$->typemask == BT_Signed || $$->typemask == BT_UnSigned)
                    {
                        $$->typemask |= $3->typemask;
                    }
                    else
                ...
                else if ($3->typemask & BT_Int)
                {
                    if ($$->typemask & BT_Long
                     || $$->typemask & BT_Short
                     || $$->typemask & BT_UnSigned
                     || $$->typemask & BT_Signed)
                    {
...
initializer_list:  initializer_reentrance
        {
            $$ = new ArrayConstant(currentLocation);
            $$->addElement($1);
        }
...
struct_or_union_definition:
        {  $$ = new StructDef();
           yywarn("ANSI/ISO C prohibits empty struct/union");
                          ...
                          | field_list
                          | PP_DIR field_list  // FIXME - for now, we just throw away the pp directive
        {
            $$ = $2;
        }
        ;
...
enum_const_def:  enum_constant
        {
            $$ = new EnumConstant($1,NULL,currentLocation);
            if (gProject->Parse_TOS->transUnit)
            {
        ...
        {
            $$ = new EnumConstant($1,$3,currentLocation);
            if (gProject->Parse_TOS->transUnit)
            {
              if (gProject->Parse_TOS->transUnit->contxt.syms->IsDefined($1->name))
                 yyerr("Duplicate enumeration constant");
...
field_list_reentrance:  comp_decl SEMICOLON
          ...
          |  field_list_reentrance comp_decl SEMICOLON
        {
          ...
          | field_list_reentrance PP_DIR   // FIXME - for now, we just delete pp tokens appearing here - should really not merge pp grammar with main grammar
        {
            $$ = $1;
        }
        ;
...
gcc_attrib_single:    ATTRIBUTE LPAREN LPAREN gcc_inner RPAREN RPAREN
            {
                $$ = $4;
            }
            ;

gcc_inner:  UNKNOWN_GCC_ATTRIBUTE
            {
         ...
         | NONNULL LPAREN inum_list RPAREN
            {
                $$ = new GccAttrib( GCC_NonNull );
                $$->ns = $3;
            }
         | PURE
            {
         ...
         | WARN_UNUSED_RESULT
            {
                $$ = new GccAttrib(GCC_WarnUnusedResult);
            }
            ;
            
kernel_attrib_single:    KERNEL_ATTRIBUTE LBRCKT kernel_inner RBRCKT
            {
                $$ = $3;
            }
            ;

kernel_inner:  UNKNOWN_KERNEL_ATTRIBUTE
            {
                yywarn("Unsupported GCC attribute");
                $$ = new GccAttrib( GCC_Unsupported);
            }
         | GROUPSIZE LPAREN inum_list RPAREN
            {
                $$ = new GccAttrib( Kernel_GroupSize );
                $$->ns = $3;
            }
            ;
...
inum_list : INUM
              {
                  IntConstant *iCons = (IntConstant*) $1;
                  $$ = new INumList(iCons->lng);
              }
          | inum_list COMMA INUM
              {
                  IntConstant *iCons = (IntConstant*) $3;
                  $$ = new INumList(iCons->lng);
                  $$->next = $1;
