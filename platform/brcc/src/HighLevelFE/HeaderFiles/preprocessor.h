...
{ 
  ...
  //find start
  while(start!=end && IsWs(start[0])) ++start;

  //find end
  while(start!=end && IsWs(end[-1])) --end;

  return std::string(start,end);
}
typedef enum _IfTokenType
{
    BRPP_ConstIntType = 0,
    BRPP_IdentifierType
}IfTokenType;
...
public:
  Preprocessor() 
    : mNestedIfDefCount(0), mDisabledCount(0) 
  {}

  //Callbacks to handle #xxxxxx
  //yytext is commented out in case it is a macro in *.l file
  void HandleDefine(const char* /*yytext*/);
  void HandleUndef(const char* /*yytext*/);
  void HandleIf(const char* /*yytext*/, IfTokenType /*tokenType*/);
  void HandleIfdef(const char* /*yytext*/);
  void HandleIfndef(const char* /*yytext*/);
  void HandleIf(bool evalRst);
  void HandleElse(const char* /*yytext*/);
  void HandleEndif(const char* /*yytext*/);
  void HandleLine(const char* /*yytext*/);
  void HandleInclude(const char* /*yytext*/);
  void HandleUnknownDirective(const char* /*yytext*/);
  void PrintErrorMsg(const char* msg);
  void PrintWarningMsg(const char* msg);

  void AtStartOfBrFile();
  void AtEndOfBrFile();


  bool IsActive() const { return mDisabledCount==0; }

  bool ShouldReplaceSymbol( const std::string& sym ) const;

  std::string ReplaceSymbol( const std::string& sym, int (*getNextChar ) (), int& numReplacements) const;

protected:

  //this method parses a single preprocessor line into a name and a value
  StringPair parsePreprocessorLine(const char* c);

  StringMap buildMacroTable( const char*& macroiterator , const std::list<std::string>& argsValues ) const;

  std::string translateMacroValue(const char*& macroiterator, const StringMap& argTable, int& numReplacements ) const;
private:
  //counts of how many levels of ifdef scope we are in
  int mNestedIfDefCount;
  int mDisabledCount;

  //map of all currently pound-defined preprocessor symbols
  StringMap mDefinedSymbols;
};
