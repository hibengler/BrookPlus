...
{
  LOG(LOG_FN_ENTRY, "indexOfKeyword(%p)\n", e);
    ...
    if (e->etype == ET_BrtIndexofExpr)
    {
        ...
        if (se && currentArgs.find(v->name->name) != currentArgs.end() &&
            ((se->uVarDecl && (se->uVarDecl->isStream() 
                              || (se->uVarDecl->isArray() && (se->uVarDecl->form->getQualifiers() & TQ_Out) !=0)))
             || !se->uVarDecl))
        {
...
{
    LOG(LOG_FN_ENTRY, "addCurrentArgs(%p)\n", fd);
...
{
    LOG(LOG_FN_ENTRY, "needIndexOf(%p)\n", fd);
...
{
    LOG(LOG_FN_ENTRY, "CalleesClosure()\n");
    ...
    while (changed)
    {
        ...
        {
            size_t len = (*i).second.callees.size();
            ...
            for (j = (*i).second.callees.begin(); j != (*i).second.callees.end(); ++j)
            {
                if (*j != (*i).first)
                {
                    ...
                    for (std::set < std::string >::iterator k = othset->begin(); k != othset->end(); ++k)
                    {
                        output.insert(*k);
                    }
                }
...
{
    LOG(LOG_FN_ENTRY, "callIndexOf(%p)\n", e);
...
{
    LOG(LOG_FN_ENTRY, "recursiveNeedIndexOf(%p)\n", fd);
...
void Brook2Cpp_IdentifyIndexOf(TransUnit * tu)
{
    LOG(LOG_FN_ENTRY, "IdentifyIndexOf(%p)\n", tu);
...
{
    LOG(LOG_FN_ENTRY, "getIndexOfDecl(%s, %d)\n", name.c_str(), baset);
...
{
    LOG(LOG_FN_ENTRY, "changeFunctionCallToPassIndexOf(%p)\n");
...
{
    LOG(LOG_FN_ENTRY, "changeFunctionCallForIndexOf(%p)\n", fd);
