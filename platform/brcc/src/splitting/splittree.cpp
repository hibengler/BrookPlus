...
SplitTree::SplitTree(FunctionDef * inFunctionDef, const SplitCompiler & inCompiler):_pseudoRoot(NULL), _resultValue(NULL),
...
SplitTree::SplitTree(FunctionDef * inFunctionDef,
                     ...
                     const std::vector < SplitNode * >&inArguments):_pseudoRoot(NULL), _resultValue(NULL),
_compiler(inCompiler)
{
...
void SplitTree::printTechnique(const SplitTechniqueDesc & UNUSED(inTechniqueDesc), std::ostream & inStream)
{
    // TIM: sanity check:
...
void SplitTree::rdsMergePasses(bool UNUSED(inLastTime))
{
...
{
    if (inDescenent->isMarked(SplitNode::kMarkBit_Descendent))
        return;
...
{
    if (inAncestor->isMarked(SplitNode::kMarkBit_Ancestor))
        return;
...
//        std::cout << "skipping !!!" << std::endl;
            ...
            if (node->_wasConsideredSave
                && node->_wasConsideredRecompute && (node->_wasSavedSave == node->_wasSavedRecompute))
            {
...
bool SplitTree::rdsMergeSome(SplitNode * n, const NodeList & inUnsavedChildren, size_t inSubsetSize,
                             SplitShaderHeuristics & outHeuristics)
{
...
//    dumpFile << "subset chosen was " << subsetBitfield << std::endl;
        ...
        for (size_t i = 0; i < unsavedChildCount; i++)
            unsavedChildren[i]->_splitHere = (subsetBitfield & (one << i)) == 0;
