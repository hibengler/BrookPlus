...
SplitTreeBuilder::SplitTreeBuilder(SplitTree & ioTree):tree(ioTree), compiler(ioTree.getComplier()), _resultValue(NULL)
{
}
...
SplitNode *SplitTreeBuilder::addArgument(Decl * inDeclaration, int UNUSED(inArgumentIndex), SplitNode * inValue)
{
    std::string name = inDeclaration->name->name;
    Type *type = inDeclaration->form;
    TypeQual quals = type->getQualifiers();
...
SplitNode *SplitTreeBuilder::addConstructor(SplitBasicType inType, SplitNode * inX, SplitNode * inY, SplitNode * inZ,
                                            SplitNode * inW)
{
    assert(inX);
...
SplitNode *SplitTreeBuilder::addConditional(SplitNode * inCondition, SplitNode * inConsequent, SplitNode * inAlternate)
{
    ...
    assert(inConsequent);
    assert(inAlternate);
    SplitNode *result = new ConditionalSplitNode(inCondition->getValueNode(), inConsequent->getValueNode(),
                                                 inAlternate->getValueNode());
