...
char *Subprocess_Run(const char * const argv[], const char *input)
{
...
#if _WIN32
    if ((pid = (int) _spawnvp(P_NOWAIT, argv[0], argv)) == -1)
    {
        if (debug)
    ...
    if (debug)
        fprintf(stderr, "Sending the input to %s\n", argv[0]);

    if (input)
    {
#if _WIN32
        _write(hStdInPipe[WRITE_HANDLE], input, (unsigned int) strlen(input));
#else
        {
            char eof_holder = EOF;
            int retval;
    ...
    if (debug)
        fprintf(stderr, "Closing pipe to %s\n", argv[0]);
    ...
    while (1)
    {
        ...
        if (debug)
            fprintf(stderr, "Reading pipe from %s...\n", argv[0]);
#ifdef _WIN32
        ...
        {
            if (debug)
                fprintf(stderr, "Got everything from %s.\n", argv[0]);
            break;
        }
        else if (ret == -1)
        {
